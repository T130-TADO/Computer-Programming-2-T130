...
float unit(float chrg, float unit) {
    if (unit>199) chrg=1.50; if(unit>=250) chrg=1.60; if(unit <400) chrg=1.60;
    if(unit>=450) chrg=1.85; if(unit <600) chrg=1.85; if (unit>=600 ) chrg=2.00;
    return chrg; }

float Unit_charge(float a, float b) {
    float result; result = a*b;
    return result; }
float surcharge( float bill) {
    float equivalent; if (bill>400) equivalent=bill/10;
    else equivalent= 100;
    return equivalent; }
